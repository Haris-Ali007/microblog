...
# Both foreign keys combined will create a compound primary key
followers = sa.Table(
    'followers',
    db.metadata,
...
class PaginatedAPIMixin(object):
    ...
    def to_collection_dict(query, page, per_page, endpoint, **kwargs):
        ...
        data = {
            ...
            '_links': {
                'self': url_for(endpoint, page=page, per_page=per_page, **kwargs), # rest are argument for the endpoint
...
# so.Mapped[int] makes the values non-nullable or required.
# for nullable we can use Optional
class User(UserMixin, PaginatedAPIMixin, db.Model):
    ...
    # this will fetch posts from all the users 
    # user is following
    def following_posts(self):
        Follower = so.aliased(User)
        Author = so.aliased(User)
        # outer join to select authors with no followers too (right table null results)
        return (
            sa.select(Post)
    ...
    # reverse mapping from dict to userdata
    def from_dict(self, data, new_user=False):
        for field in ['username', 'email', 'about_me']:
            if field in data:
                setattr(self, field, data[field]) # sets self.field = data['field']
